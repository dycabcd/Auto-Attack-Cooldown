...
/**
 * 自定义 Forge 配置界面（双列紧凑布局）。
 */
@OnlyIn(Dist.CLIENT)
public class AutoAttackConfigScreen {

    public static void register() {
        net.minecraftforge.common.MinecraftForge.registerConfigScreen(
                (Minecraft client, Screen parent) -> new ConfigScreenImpl(parent)
        );
    }

    @OnlyIn(Dist.CLIENT)
    private static class ConfigScreenImpl extends Screen {
        ...
        protected ConfigScreenImpl(Screen parent) {
        ...
        private static void saveConfig() {
            if (AutoAttackConfig.modConfig != null) AutoAttackConfig.modConfig.save();
        }
